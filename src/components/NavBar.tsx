    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between gap-4">
        ...
        <div className="hidden md:flex items-center gap-1.5 shrink-0">
          ...
          <button
            aria-label="Language"
            className="flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-sm font-medium text-slate-400 hover:text-white hover:bg-white/10 transition-all"
          >
          ...
          <Link
            href="/register"
            className="px-4 py-2 bg-violet-600 hover:bg-violet-500 text-white text-sm font-bold rounded-lg transition-all hover:-translate-y-px shadow-md shadow-violet-500/30 hover:shadow-violet-500/50"
          >
            Get started free
          </Link>
        </div>
        ...
        >
          ...
          <span className={`block w-5 h-px bg-current transition-all ${menuOpen ? "opacity-0 scale-x-0" : ""}`} />
          <span className={`block w-5 h-px bg-current transition-all origin-center ${menuOpen ? "-rotate-45 -translate-y-[7px]" : ""}`} />
        </button>
      </div>
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div className="md:hidden border-t border-white/5 bg-slate-900/98 px-6 py-4 flex flex-col gap-1">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={[
                "px-3.5 py-2.5 rounded-lg text-sm font-medium transition-all",
                isActive(link.href)
                  ? "text-white bg-white/10"
          ...
          <div className="mt-3 pt-3 border-t border-white/5 flex flex-col gap-2">
            <Link
              href="/login"
              className="px-3.5 py-2.5 text-sm font-semibold text-slate-300 hover:text-white rounded-lg hover:bg-white/10 transition-all"
            >
              Log in
            </Link>
            <Link
              href="/register"
              className="px-3.5 py-2.5 bg-violet-600 text-white text-sm font-bold rounded-lg text-center hover:bg-violet-500 transition-colors"
            >
              Get started free
            </Link>
          </div>
