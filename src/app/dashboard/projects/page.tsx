...
import {
  Plus, Search, MoreHorizontal, Users, Calendar,
  TrendingUp, X, Trash2, RefreshCw, Edit2, Check,
} from "lucide-react";
import {
  useGetProjectsQuery,
...
const STATUS_LABEL: Record<string, string> = {
  in_progress: "In Progress", almost_done: "Almost Done", planning: "Planning", active: "Active",
};
const PROJECT_COLORS = [
  "#0052cc", "#5e4db2", "#f6339a", "#ff6900",
  "#00875a", "#00a3bf", "#de350b", "#2b7fff", "#9810fa",
];
...
function NewProjectModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState<CreateForm>({
    name: "", description: "", startDate: "", dueDate: "", color: "#0052cc", teamId: "",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createProject(payload as Parameters<typeof createProject>[0]).unwrap();
      onClose();
    } catch (err: unknown) {
    ...
    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-[#f4f5f7]">
          ...
          <button onClick={onClose} className="w-7 h-7 rounded-md flex items-center justify-center text-[#97a0af] hover:bg-[#f4f5f7] transition-colors"><X size={15} /></button>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {apiError && <p className="text-sm text-[#de350b] bg-[#ffeceb] border border-[#ffd5cc] p-3 rounded-lg">{apiError}</p>}
          <div>
            <label className="block text-sm font-semibold text-[#44526e] mb-1.5">Project name *</label>
            <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Website Redesign"
              className={`w-full h-10 px-3 rounded-md border text-sm outline-none bg-white transition-colors ${errors.name ? "border-[#de350b]" : "border-[#dfe1e6] focus:border-[#0052cc]"}`} />
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-semibold text-[#44526e] mb-1.5">Start date</label>
          ...
          {teams.length > 0 && (
            <div>
              <label className="block text-sm font-semibold text-[#44526e] mb-1.5">Team</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-[#44526e] mb-2">Color</label>
            <div className="flex gap-2 flex-wrap">
              {PROJECT_COLORS.map((c) => (
                <button key={c} type="button" onClick={() => setForm({ ...form, color: c })}
                  className={`w-7 h-7 rounded-full transition-transform ${form.color === c ? "ring-2 ring-offset-2 ring-[#0052cc] scale-110" : "hover:scale-105"}`}
...
function EditProjectModal({ project, onClose }: { project: Project; onClose: () => void }) {
  ...
  const [form, setForm] = useState<UpdateForm>({
    ...
    dueDate: project.dueDate ?? "", color: project.color,
  });
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-[#44526e] mb-1.5">Description</label>
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
              rows={2} className="w-full px-3 py-2.5 rounded-md border border-[#dfe1e6] text-sm outline-none focus:border-[#0052cc] bg-white resize-none transition-colors" />
          </div>
          <div>
            <label className="block text-sm font-semibold text-[#44526e] mb-1.5">Due date</label>
            <input type="date" value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })}
...
function ProjectCard({
  ...
  return (
    <div className="bg-white rounded-xl border border-[#ebecf0] overflow-hidden hover:shadow-md transition-all group">
      <div className="h-2" style={{ backgroundColor: project.color }} />
      <div className="p-5 space-y-4">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-md flex items-center justify-center text-white text-xs font-bold shrink-0" style={{ backgroundColor: project.color }}>
              {project.name[0].toUpperCase()}
            </div>
            <div>
              <h3 className="text-sm font-bold text-[#172b4d] leading-tight">{project.name}</h3>
              {project.description && (
                <p className="text-xs text-[#97a0af] mt-0.5 line-clamp-1">{project.description}</p>
              )}
            </div>
          </div>
          <div className="relative">
            <button onClick={() => setMenuOpen((o) => !o)} className="w-7 h-7 flex items-center justify-center text-[#97a0af] hover:bg-[#f4f5f7] rounded opacity-0 group-hover:opacity-100 transition-all">
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute right-0 top-8 z-20 bg-white rounded-lg shadow-lg border border-[#ebecf0] py-1 min-w-[120px]">
                  ...
                  <button onClick={() => { if (confirm("Delete this project?")) { onDelete(project.id); setMenuOpen(false); } }}
        ...
        {project.status && (
          <span className={`inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full border ${STATUS_STYLE[statusKey] ?? STATUS_STYLE.active}`}>
        ...
        <div className="flex items-center justify-between text-xs text-[#6b778c]">
          <span className="flex items-center gap-1"><TrendingUp size={11} /> {project.tasksDone ?? 0}/{project.tasksCount ?? 0} tasks</span>
          {project.members && project.members.length > 0 && (
            <span className="flex items-center gap-1"><Users size={11} /> {project.members.length}</span>
          )}
          {project.dueDate && (
            <span className="flex items-center gap-1"><Calendar size={11} /> {new Date(project.dueDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
          )}
        </div>
      </div>
    </div>
    ...
    <>
      <header className="h-16 bg-white border-b border-[#ebecf0] flex items-center justify-between px-6 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowNew(true)} className="flex items-center gap-2 h-9 px-4 rounded-md bg-[#0052cc] text-white text-sm font-semibold hover:bg-[#0041a3] transition-colors">
      ...
      {isError && (
        <div className="mx-6 mt-4 p-3 rounded-lg bg-red-50 border border-red-200 text-sm text-red-600 flex items-center justify-between">
          Failed to load projects. <button onClick={refetch} className="font-semibold underline">Retry</button>
        </div>
      )}
      ...
      <main className="flex-1 overflow-auto p-6">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-center">
            <p className="text-[#44526e] font-semibold text-sm">No projects found</p>
            <p className="text-[#97a0af] text-xs mt-1">Create a project to get started</p>
            <button onClick={() => setShowNew(true)} className="mt-4 flex items-center gap-2 h-9 px-4 rounded-md bg-[#0052cc] text-white text-sm font-semibold hover:bg-[#0041a3] transition-colors">
              <Plus size={15} /> New Project
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((p) => (
              <ProjectCard key={p.id} project={p} onEdit={setEdit} onDelete={(id) => deleteProject(id)} />
            ))}
          </div>
        )}
      </main>
      ...
      {editProject && <EditProjectModal project={editProject} onClose={() => setEdit(null)} />}
    </>
  );
}
